...
mod tests {
    ...
    #[test]
    fn collect_function_calls_ignores_invalid_identifier_heads() {
    ...
    fn collect_function_calls_in_template_dedupes_and_aggregates() {
        let calls = collect_function_calls_in_template(
            r#"a{{ include "x" . }}b{{ default "y" .Values.name | quote }}c{{ include "x" . }}"#,
        );
        assert_eq!(
            calls,
            vec![
